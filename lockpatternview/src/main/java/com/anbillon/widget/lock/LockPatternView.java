...
public final class LockPatternView extends ViewGroup {
	...
	private Paint paint;
	private final List<NodeView> nodeCache = new ArrayList<>();
	...
	@Override protected void onDraw(Canvas canvas) {
		...
		if (nodeCache.size() > 0) {
			...
			canvas.drawLine(lastNode.getCenterX(), lastNode.getCenterY(), currentX, currentY,
				paint);
		}
	}
	...
	private void init(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		...
		int nodeOnAnim = a.getResourceId(R.styleable.LockPatternView_nodeOnAnim, 0);
		a.recycle();
    ...
    /* add all node into current ViewGroup */
		...
		for (int i = 0; i < totalNodes; i++) {
			...
			nodeView.setAnimResId(nodeOnAnim);
	...
	private NodeView getNodeAt(float x, float y) {
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (!(x >= node.getLeft() - nodeAreaExpand && x < node.getRight() + nodeAreaExpand)
				|| !(y >= node.getTop() - nodeAreaExpand
				&& y < node.getBottom() + nodeAreaExpand)) {
				continue;
			}
	...
	private void handleCallbak() {
		...
		for (NodeView nodeView : nodeCache) {
			passwordBuilder.append(nodeView.getNodeNumber());
		}

		onPatternCompleteListener.onPatternComplete(passwordBuilder.toString());
	}
